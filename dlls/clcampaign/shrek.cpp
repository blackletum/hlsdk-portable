...
#define		SHREK_SPRINT_DIST	256 // how close the squid has to get before starting to sprint and refusing to swerve
//iShrekSpitSprite & iSquidSpitSprite; CshrekSpit & CSquidSpit
int iShrekSpitSprite;
...
enum
{
	SCHED_SHREK_HURTHOP = LAST_COMMON_SCHEDULE + 1,
	SCHED_SHREK_SMELLFOOD,
	SCHED_SHREK_SEECRAB,
	SCHED_SHREK_EAT,
	SCHED_SHREK_SNIFF_AND_EAT,
	SCHED_SHREK_WALLOW
};
...
enum 
{
	TASK_SHREK_HOPTURN = LAST_COMMON_TASK + 1
};

//=========================================================
// Bullsquid's spit projectile
//=========================================================
class CShrekSpit : public CBaseEntity
{
public:
	void Spawn( void );
...
LINK_ENTITY_TO_CLASS( shrekspit, CShrekSpit )

TYPEDESCRIPTION	CShrekSpit::m_SaveData[] =
{
	DEFINE_FIELD( CShrekSpit, m_maxFrame, FIELD_INTEGER ),
};

IMPLEMENT_SAVERESTORE( CShrekSpit, CBaseEntity )

void CShrekSpit::Spawn( void )
{
	pev->movetype = MOVETYPE_FLY;
	pev->classname = MAKE_STRING( "shrekspit" );
...
void CShrekSpit::Shoot( entvars_t *pevOwner, Vector vecStart, Vector vecVelocity )
{
	CShrekSpit *pSpit = GetClassPtr( (CShrekSpit *)NULL );
	pSpit->Spawn();
	...
	pSpit->SetThink( &CShrekSpit::Animate );
	pSpit->pev->nextthink = gpGlobals->time + 0.1;
}

void CShrekSpit::Touch( CBaseEntity *pOther )
{
	TraceResult tr;
	int iPitch;
...
//=========================================================
#define		BSHREK_AE_SPIT		( 1 )
#define		BSHREK_AE_BITE		( 2 )
#define		BSHREK_AE_BLINK		( 3 )
#define		BSHREK_AE_TAILWHIP	( 4 )
#define		BSHREK_AE_HOP		( 5 )
#define		BSHREK_AE_THROW		( 6 )

class CShrek : public CBaseMonster
{
public:
	void Spawn( void );
	void Precache( void );
...
LINK_ENTITY_TO_CLASS( monster_shrek, CShrek )

TYPEDESCRIPTION	CShrek::m_SaveData[] =
{
	DEFINE_FIELD( CShrek, m_fCanThreatDisplay, FIELD_BOOLEAN ),
	DEFINE_FIELD( CShrek, m_flLastHurtTime, FIELD_TIME ),
	DEFINE_FIELD( CShrek, m_flNextSpitTime, FIELD_TIME ),
};

IMPLEMENT_SAVERESTORE( CShrek, CBaseMonster )

//=========================================================
// IgnoreConditions 
//=========================================================
int CShrek::IgnoreConditions( void )
{
...
//=========================================================
int CShrek::IRelationship( CBaseEntity *pTarget )
{
...
//=========================================================
int CShrek::TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType )
{
	float flDist;
	Vector vecApex;
...
// CheckRangeAttack1
//=========================================================
BOOL CShrek::CheckRangeAttack1( float flDot, float flDist )
{
...
// is valid.
//=========================================================
BOOL CShrek::CheckMeleeAttack2( float flDot, float flDist )
{
...
//  FValidateHintType 
//=========================================================
BOOL CShrek::FValidateHintType( short sHint )
{
	int i;

	static short sShrekHints[] =
	{
		HINT_WORLD_HUMAN_BLOOD,
	};

	for( i = 0; i < ARRAYSIZE( sShrekHints ); i++ )
	{
		if( sShrekHints[i] == sHint )
		{
			return TRUE;
		}
	}
...
// IdleSound 
//=========================================================
#define SHREK_ATTN_IDLE	(float)1.5
void CShrek::IdleSound( void )
{
...
//=========================================================
void CShrek::HandleAnimEvent( MonsterEvent_t *pEvent )
{
	switch( pEvent->event )
	{
		case BSHREK_AE_SPIT:
			{
				...
				CShrekSpit::Shoot( pev, vecSpitOffset, vecSpitDir * 900 );
			}
			break;
		case BSHREK_AE_BITE:
			{
...
void CShrek::Spawn()
{
	...
	SET_MODEL( ENT( pev ), "models/shrek.mdl" );
...
//=========================================================
void CShrek::Precache()
{
	PRECACHE_MODEL( "models/shrek.mdl" );

	PRECACHE_MODEL( "sprites/bigspit.spr" );// spit projectile.

	iShrekSpitSprite = PRECACHE_MODEL( "sprites/tinyspit.spr" );// client side spittle.
	...
	PRECACHE_SOUND( "shrek/bc_idle1.wav" );
...
//Testbag
Schedule_t slShrekRangeAttack1[] =
{
	{
		tlShrekRangeAttack1,
		ARRAYSIZE( tlShrekRangeAttack1 ),
		bits_COND_NEW_ENEMY |
...
Schedule_t slShrekChaseEnemy[] =
{
	{
		tlShrekChaseEnemy1,
		ARRAYSIZE( tlShrekChaseEnemy1 ),
		bits_COND_NEW_ENEMY |
		bits_COND_ENEMY_DEAD |
...
Schedule_t slShrekSeeCrab[] =
{
	{
		tlShrekSeeCrab,
		ARRAYSIZE( tlShrekSeeCrab ),
...
Schedule_t slShrekEat[] =
{
	{
		tlShrekEat,
		ARRAYSIZE( tlShrekEat ),
		bits_COND_LIGHT_DAMAGE |
		bits_COND_HEAVY_DAMAGE |
		bits_COND_NEW_ENEMY,
...
Schedule_t slShrekSniffAndEat[] =
{
	{
		tlShrekSniffAndEat,
		ARRAYSIZE( tlShrekSniffAndEat ),
		bits_COND_LIGHT_DAMAGE |
		bits_COND_HEAVY_DAMAGE |
		bits_COND_NEW_ENEMY,
		// even though HEAR_SOUND/SMELL FOOD doesn't break this schedule, we need this mask
...
Schedule_t slShrekWallow[] =
{
	{
		tlShrekWallow,
		ARRAYSIZE( tlShrekWallow ),
		bits_COND_LIGHT_DAMAGE |
		bits_COND_HEAVY_DAMAGE |
		bits_COND_NEW_ENEMY,
		// even though HEAR_SOUND/SMELL FOOD doesn't break this schedule, we need this mask
...
DEFINE_CUSTOM_SCHEDULES( CShrek ) 
{
	slShrekRangeAttack1,
	slShrekChaseEnemy,
	slShrekHurtHop,
...
IMPLEMENT_CUSTOM_SCHEDULES( CShrek, CBaseMonster )

//=========================================================
// GetSchedule 
//=========================================================
Schedule_t *CShrek::GetSchedule( void )
{
	...
	case MONSTERSTATE_ALERT:
		{
			...
			{
				return GetScheduleOfType( SCHED_SHREK_HURTHOP );
			}
			...
			if( HasConditions( bits_COND_SMELL_FOOD ) )
			{
				...
				if( pSound && ( !FInViewCone( &pSound->m_vecOrigin ) || !FVisible( pSound->m_vecOrigin ) ) )
				{
					...
					return GetScheduleOfType( SCHED_SHREK_SNIFF_AND_EAT );
				}

				// food is right out in the open. Just go get it.
				return GetScheduleOfType( SCHED_SHREK_EAT );
			}
			...
			if( HasConditions( bits_COND_SMELL ) )
			{
				...
				pSound = PBestScent();
				if( pSound )
					return GetScheduleOfType( SCHED_SHREK_WALLOW );
			}
			break;
		}
	case MONSTERSTATE_COMBAT:
		{
			...
			if( HasConditions( bits_COND_NEW_ENEMY ) )
			{
				if( m_fCanThreatDisplay && IRelationship( m_hEnemy ) == R_HT )
				{
					...
					return GetScheduleOfType( SCHED_SHREK_SEECRAB );
				}
				else
				{
...
// GetScheduleOfType
//=========================================================
Schedule_t *CShrek::GetScheduleOfType( int Type ) 
{
	switch( Type )
	{
	case SCHED_RANGE_ATTACK1:
		return &slShrekRangeAttack1[0];
		break;
	case SCHED_SHREK_HURTHOP:
		return &slShrekHurtHop[0];
		break;
	case SCHED_SHREK_SEECRAB:
		return &slShrekSeeCrab[0];
		break;
	case SCHED_SHREK_EAT:
		return &slShrekEat[0];
		break;
	case SCHED_SHREK_SNIFF_AND_EAT:
		return &slShrekSniffAndEat[0];
		break;
	case SCHED_SHREK_WALLOW:
		return &slShrekWallow[0];
		break;
	case SCHED_CHASE_ENEMY:
		return &slShrekChaseEnemy[0];
		break;
	}
...
//=========================================================
void CShrek::StartTask( Task_t *pTask )
{
...
// RunTask
//=========================================================
void CShrek::RunTask( Task_t *pTask )
{
	switch( pTask->iTask )
	{
	case TASK_SHREK_HOPTURN:
...
//=========================================================
MONSTERSTATE CShrek::GetIdealState( void )
{
	int iConditions;
